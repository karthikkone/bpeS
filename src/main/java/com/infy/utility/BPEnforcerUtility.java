...
public class BPEnforcerUtility {
	...
	public static String insertScanDtls(String organisationID, String organisationName,String remoteAuth ) throws ConnectionException {
		...
		SObject scanObj = new SObject();
			...
			Integer lines = DataStore.TOTALLINESOFORG;
			
			scanObj.setField(DataStore.NAMESPACE+"Total_lines__c", lines);
			System.out.println(lines);
			
			SimpleDateFormat dateFormat = new SimpleDateFormat("MMM_yyyy");
			Calendar calendar = Calendar.getInstance();
			String scanDate=dateFormat.format(calendar.getTime()).toString();
			System.out.println(scanDate);
			scanObj.setField(DataStore.NAMESPACE+"Duration__c", scanDate);
		...
		System.out.println("@@@@@@@@@@@@@@@"+scanSaveResults[0].getId());
