 ...
 *
 */
public class ApexCRUDViolationCustomRule extends AbstractApexRule {
    ...
    public ApexCRUDViolationCustomRule() {
    ...
    private void validateCRUDCheckPresent(final AbstractApexNode<?> node, final Object data, final String crudMethod,
        ...
        if (!typeToDMLOperationMapping.containsKey(typeCheck)) {
            if (!isProperESAPICheckForDML(typeCheck, crudMethod)) {
            	...
            	CodeAnalyser.addBestPracticesRule(ToolingOperations.setClassReportDtls(node.getNode().getDefiningType().getApexName(), CategoryConstants.APEX_CRUD_VIOLATION, "Apex CRUD Violation found ",node.getBeginLine(),BPEnforcerConstants.HIGH, BPEnforcerConstants.CRUD_VIOLATION,"MEDIUM",BPEnforcerConstants.SECURITY));
