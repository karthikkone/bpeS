 ...
 *
 */
public class ApexDangerousMethodsCustomRule extends AbstractApexRule {
    ...
    public ApexDangerousMethodsCustomRule() {
    ...
    public Object visit(ASTUserClass node, Object data) {
        ...
        for (ASTMethodCallExpression methodCall : methodCalls) {
            if (Helper.isMethodName(methodCall, CONFIGURATION, DISABLE_CRUD)) {
            	...
            	CodeAnalyser.addBestPracticesRule(ToolingOperations.setClassReportDtls(node.getNode().getDefiningType().getApexName(), CategoryConstants.APEX_DANGEROUS_METHODS, "Apex Dangerous method found which may cause security risks ",node.getBeginLine(),BPEnforcerConstants.HIGH, BPEnforcerConstants.DANGEROUS_METHODS,"MEDIUM",BPEnforcerConstants.SECURITY));
        ...
        for (ASTVariableExpression var : variables) {
            if (REGEXP.matcher(var.getNode().getIdentifier().getValue()).matches()) {
                if (!whiteListedVariables.contains(Helper.getFQVariableName(var))) {
                	...
                	CodeAnalyser.addBestPracticesRule(ToolingOperations.setClassReportDtls(methodCall.getNode().getDefiningType().getApexName(), CategoryConstants.APEX_DANGEROUS_METHODS, "Apex Dangerous method found which may cause security risks ",methodCall.getBeginLine(),BPEnforcerConstants.HIGH, BPEnforcerConstants.DANGEROUS_METHODS,"MEDIUM",BPEnforcerConstants.SECURITY));
                	// addViolation(data, methodCall);
                }
            }
        }
