    ...
    private void getObjectValue(ApexNode<?> node, Object data) {
        ...
        for (ASTVariableExpression variable : variableExpressions) {
            if (variable.jjtGetChildIndex() == 0
            	...
            	CodeAnalyser.addBestPracticesRule(ToolingOperations.setClassReportDtls(node.getNode().getDefiningType().getApexName(), CategoryConstants.APEX_OPEN_REDIRECT, "Apex Open Redirect ",node.getBeginLine(),BPEnforcerConstants.HIGH, BPEnforcerConstants.OPEN_REDIRECT,"MEDIUM",BPEnforcerConstants.SECURITY));
