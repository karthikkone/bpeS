 ...
 *
 */
public class ApexBadCryptoCustomRule extends AbstractApexRule {
    ...
    public ApexBadCryptoCustomRule() {
    ...
    private void reportIfHardCoded(Object data, Object potentialIV) {
        ...
        if (potentialIV instanceof ASTVariableExpression) {
            ...
            if (potentiallyStaticBlob.contains(Helper.getFQVariableName(variable))) {
            	...
            	//System.out.println(variable.getNode().getDefiningType().getApexName());
            	//System.out.println(variable.getBeginLine());
            	//System.out.println (variable.getEndLine());
            	
               	CodeAnalyser.addBestPracticesRule(ToolingOperations.setClassReportDtls(variable.getNode().getDefiningType().getApexName(), CategoryConstants.APEX_BAD_CRYPTO, "Apex Bad Crypto found which may compromise the security of encrypted data ",variable.getBeginLine(),BPEnforcerConstants.HIGH, BPEnforcerConstants.BAD_CRYPTO,"MEDIUM",BPEnforcerConstants.SECURITY));
