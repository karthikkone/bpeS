 ...
 *
 */
public class ApexSOQLInjectionCustomRule extends AbstractApexRule {
	...
	public ApexSOQLInjectionCustomRule() {
		...
		for (ASTBinaryExpression b : binaryExpr) {
			...
			for (ASTVariableExpression v : vars) {
				...
				if (!isSafeMethod) {
					...
					// System.out.println(parentCall.getNode().getDefiningType().getApexName());
					// System.out.println(parentCall.getBeginLine());
					// System.out.println (parentCall.getEndLine());
					CodeAnalyser.addBestPracticesRule(ToolingOperations.setClassReportDtls(
							parentCall.getNode().getDefiningType().getApexName(), CategoryConstants.APEX_SOQL_INJECTION,
							"Detected the usage of untrusted / unescaped variables in DML queries",
							parentCall.getBeginLine(), BPEnforcerConstants.HIGH, BPEnforcerConstants.SOQL_INJECTION,
							"MEDIUM", BPEnforcerConstants.SECURITY));
					// addViolation(data, v);
				}
			}
		}
	...
	private void reportVariables(final ASTMethodCallExpression m, Object data) {
		...
		if (var != null) {
			...
			if (selectContainingVariables.containsKey(nameFQ)) {
				...
				if (!isLiteral) {
					// System.out.println(var.getNode().getDefiningType().getApexName());
					// System.out.println(var.getBeginLine());
					// System.out.println (var.getEndLine());
					CodeAnalyser.addBestPracticesRule(ToolingOperations.setClassReportDtls(
							var.getNode().getDefiningType().getApexName(), CategoryConstants.APEX_SOQL_INJECTION,
							"Detected the usage of untrusted / unescaped variables in DML queries", var.getBeginLine(),
							BPEnforcerConstants.HIGH, BPEnforcerConstants.SOQL_INJECTION, "MEDIUM",
