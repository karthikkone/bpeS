 ...
 */
public class ApexSharingViolationsCustomRule extends AbstractApexRule {

    private WeakHashMap<ApexNode<?>, Object> localCacheOfReportedNodes = new WeakHashMap<>();

    public ApexSharingViolationsCustomRule() {
    ...
    private void reportViolation(ApexNode<?> node, Object data) {
        ...
        if (modifier != null) {
            if (localCacheOfReportedNodes.put(modifier, data) == null) {
            	...
            	CodeAnalyser.addBestPracticesRule(ToolingOperations.setClassReportDtls(node.getNode().getDefiningType().getApexName(), CategoryConstants.APEX_SHARING_VIOLATIONS, "Class detected with Apex Sharing Violation(s) ",node.getBeginLine(),BPEnforcerConstants.HIGH, BPEnforcerConstants.SHARING_VIOLATIONS,"MEDIUM",BPEnforcerConstants.SECURITY));
