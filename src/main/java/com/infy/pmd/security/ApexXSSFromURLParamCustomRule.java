 ...
 *
 */
public class ApexXSSFromURLParamCustomRule extends AbstractApexRule {
    ...
    public ApexXSSFromURLParamCustomRule() {
    ...
    public Object visit(ASTReturnStatement node, Object data) {
        ...
        for (ASTVariableExpression varExpression : nodes) {
            if (urlParameterStrings.contains(Helper.getFQVariableName(varExpression))) {
            	...
            	CodeAnalyser.addBestPracticesRule(ToolingOperations.setClassReportDtls(node.getNode().getDefiningType().getApexName(), CategoryConstants.APEXXSSFROMURLPARAM, "URL parameters are not properly escaped / sanitized to avoid XSS attacks w ",node.getBeginLine(),BPEnforcerConstants.HIGH, BPEnforcerConstants.APEX_XSS_URL,"MEDIUM",BPEnforcerConstants.SECURITY));
    ...
    private void processInlineMethodCalls(ASTMethodCallExpression methodNode, Object data, final boolean isNested) {
        ...
        if (Helper.isMethodCallChain(methodNode, URL_PARAMETER_METHOD)) {
            if (isNested) {
            	//System.out.println("vem1");
            	//System.out.println(methodNode.getNode().getDefiningType().getApexName());
            	//System.out.println(methodNode.getBeginLine());
            	//System.out.println (methodNode.getEndLine());
            	CodeAnalyser.addBestPracticesRule(ToolingOperations.setClassReportDtls(methodNode.getNode().getDefiningType().getApexName(), CategoryConstants.APEXXSSFROMURLPARAM, "URL parameters are not properly escaped / sanitized to avoid XSS attacks  ",methodNode.getBeginLine(),BPEnforcerConstants.HIGH, BPEnforcerConstants.APEX_XSS_URL,"MEDIUM",BPEnforcerConstants.SECURITY));
    ...
    private void processVariableAssignments(AbstractApexNode<?> node, Object data, final boolean reverseOrder) {
        ...
        case 2: {
            ...
            if (urlParameterStrings.contains(Helper.getFQVariableName(right))) {
            	...
            	System.out.println(node.getBeginLine());
            	System.out.println (node.getEndLine());
            	CodeAnalyser.addBestPracticesRule(ToolingOperations.setClassReportDtls(node.getNode().getDefiningType().getApexName(), CategoryConstants.APEXXSSFROMURLPARAM, "URL parameters are not properly escaped / sanitized to avoid XSS attacks ",node.getBeginLine(),BPEnforcerConstants.HIGH, BPEnforcerConstants.APEX_XSS_URL,"MEDIUM",BPEnforcerConstants.SECURITY));
